...
import { Observable, distinctUntilChanged, filter, finalize, map, take, tap } from 'rxjs';
import { CxaiAskProductService } from '../../cxai-ask-product.service';
import { AskProductChatMessage, AskProductSource } from '@cx-spartacus/cxai-ask-product/root';
...
export class CxaiAskProductChatComponent implements OnInit {
  ...
  ngOnInit() {
    if (!this.cxaiAskProductService.isConfigured()) {
      this.loggerService.warn(ASK_PRODUCT_LOG_MARKER, 'Ask product is not configured (missing url)');
  ...
  sendMessage(productCode: string) {
    ...
    this.cxaiAskProductService
      ...
      .subscribe((response) => {
        ...
        let sources: AskProductSource[] = response.answer ? (response.sources || []) : []
  ...
  loadConversation(productCode: string) {
    ...
    if (conversation && conversation.length > 2) {
      const storedConversation: AskProductChatMessage[] = JSON.parse(conversation);
